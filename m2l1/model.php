...
function formationValider($id_s){
  $pdo = cnx();
  $sql='SELECT f.id_f, titre, duree, cout, date_debut, nb_place, contenu, numero_rue, rue, ville, code_postal, raison_social, etat_f FROM formation f,formation_valide fv, adresse a, prestataire p WHERE f.id_a=a.id_a AND f.id_p=p.id_p and fv.id_f=f.id_f and etat_f="valider" AND fv.id_s = :id_s';
  $req= $pdo->prepare($sql);
  $req->bindVAlue(":id_s",$id_s);
  $req->execute();
  while($row = $req->fetch()): ?>
